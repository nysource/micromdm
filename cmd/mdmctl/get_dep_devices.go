...
func (out *depDevicesTableOutput) BasicHeader() {
	fmt.Fprintf(out.w, "SerialNumber\tModel\tDeviceFamily\tDeviceName\tProfileStatus\tProfileUUID\n")
}
...
func (cmd *getCommand) getDEPDevices(args []string) error {
	...
	for _, d := range resp.Devices {
		fmt.Fprintf(out.w, "%s\t%s\t%s\t%s\t%s\n", d.SerialNumber, d.Model, d.DeviceFamily, d.DeviceName, d.ProfileStatus, d.ProfileUUID)
	}
	return nil
}
