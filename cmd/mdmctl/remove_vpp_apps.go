...
func (cmd *removeCommand) removeVPPApp(args []string) error {
	...
	var (
		flAppID   = flagset.String("id", "", "specify the id of the vpp app for which to disassociate serials")
		flSerials = flagset.String("serials", "", "specify a list of serials to disassociate to the vpp app")
	)
	...
	manage, err := cmd.vppsvc.ManageVPPLicensesByAdamIdSrv(ctx, vpp.ManageVPPLicensesByAdamIdSrvOptions{
		AdamIDStr:                 *flAppID,
		DisassociateSerialNumbers: []string{*flSerials},
	})
	if err != nil {
		return err
	}
